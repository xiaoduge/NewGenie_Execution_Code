...
typedef struct
{
    ...
    system_timeo to4ActionDelay;
    system_timeo to4State;
...
void Modbus_QmHandler(uint8_t *pData, uint8_t ucLen)
{
   ...
   uint8_t ucDataPtr = APP_EXE_INPUT_REG_QMI_OFFSET + iChl * 3;
  ...
  //printf("iChl: %d; \r\n", iChl);
    ...
    && (APP_EXE_I4_NO == iChl))
   {
       ...
       usTemp = Display.ausInputRegs[ucDataPtr + 2];
       Disp_AddTocMeasurement(ud.f,usTemp);
   }
}
...
void appQmi_ItfProcess(Message *pMsg)
{
    ...
    {
    case 0x2:
        ucCnt = pModbus[MODBUS_CONT_POS] + 1 + MODBUS_HEADER_LEN; // 1bytes for rsplen info
        break;
    default :
        return; 
    }
   ...
   appQmi_FillSndBuf(0,(uint8_t *)pMsg->data,1); // extra byte for RS485 
#endif
...
{
    system_timeout(delay,SYS_TIMER_ONE_SHOT,delay,Modbus_CommActionDelayTimeoutHandler,NULL,&ModbusSheduler.RunBufAction.to4ActionDelay); 
}
...
void Modbus_Action4Qm(int modadr,int addr)
{
    ...
    system_timeout(150,SYS_TIMER_ONE_SHOT,0,Modbus_CommStateTimeoutHandler,(void *)modadr,&ModbusSheduler.RunBufAction.to4State); 
}
...
void Modbus_Scheduler_msg_cb(void)
{
    ...
    {
        int iLoop;
        for (iLoop = 0; iLoop < APP_TOC_SAMPLES_PER_SECOND - 1; iLoop++)
        {
            ModbusSheduler.RunBufAction.aActionBuf[ucIdx].ucActionId = MODBUS_TARGET_QUERY_QM;
            ModbusSheduler.RunBufAction.aActionBuf[ucIdx].para       = MODBUS_ADDRESS_MODULE_QM + 3; // address
            ModbusSheduler.RunBufAction.aActionBuf[ucIdx].para1      = 0x0021; // address
...
{
    ModbusSheduler.ucI4Speedup = ucSpeedup;
}
