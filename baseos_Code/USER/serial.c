...
void SerialInitPort(UINT8 ucPort)
{
    ...
    {
        stm32_gpio_cfgpin(Serial[ucPort].ucPortCtrl,MAKE_PIN_CFG(GPIO_PuPd_NOPULL,GPIO_OType_PP,GPIO_Speed_50MHz,GPIO_Mode_OUT)); // YLF: FOR RS485 CONTROL
...
UINT8  Serial_FillSndBuf(UINT8 ucPort ,UINT8 *pData,UINT16 usLength)
{
    ...
    if (UART_INT == Serial[ucPort].ucPortMode)
    {
    ...
    {
        for (usSpace = 0; usSpace < usLength; usSpace++)
        {
            USART_SendData(Serial[ucPort].UsartDef, pData[usSpace]);
            
            /* Loop until the end of transmission */
            while (USART_GetFlagStatus(Serial[ucPort].UsartDef, USART_FLAG_TC) == RESET)
            {
            }
        }
    }
...
void Serial_RetriveConfig(UINT8 ucPort)
{
    ...
    {
        Serial[ucPort].SerialConfig.ucIdleCheckThd = 10; // 20ms  is enough for modbus protocol
    }
    else if (Serial[ucPort].SerialConfig.ucIdleCheckThd < 10)
    {
        Serial[ucPort].SerialConfig.ucIdleCheckThd = 10; // 10~20ms is enought for modbus protocol
    }
